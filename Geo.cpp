...
int main()
{
    ...
    istringstream s{ "{ \"key1\": \"value1\", \"key2\": 42 }" };
    string parse_string;
    vector<string> top;
    unordered_map<string, string> con;

    auto read_string = [&parse_string](istream& input)
    {
        /*if (iscntrl(input.peek()))
        {
            getline(input, parse_string, '"');
        ...
        while (input)
        {
            if (iscntrl(input.peek()) || isalnum(input.peek()) || input.peek() == '"')
                parse_string += static_cast<char>(input.get());
            else
                break;
        }
    };

    auto loadvector = [&, read_string](istream& input)
    {
        /*if (input.peek() == '[')
        {
            input.get();
            for (char c; input >> c && c != ']';) {
                ...
                else
                {
                    ...
                    top.push_back(move(parse_string));
                }
            }
        }*/
        if (input.peek() == '{')
        {
            ...
            while (input)
            {
                 ...
                 if (iscntrl(input.peek()) || isalnum(input.peek()) || input.peek() == '"')
                 {
                     ...
                     {
                         read_string(input);
                         key = move(parse_string);
                         flag = false;
                     }
                     else
                     {
                         read_string(input);
                         con.insert({ move(key), move(parse_string) });
                         flag = true;
                     }
                 }
                 else
        ...
        else if (input.peek() == '[')
        {
            while (input)
            {
                if (input.peek() == '[' || isspace(input.peek()))
                    input.ignore();
                else if (input)
                {
                    read_string(input);
