...
namespace reader
{
    ...
    Color color(const Node& color)
    {
        ...
        else return monostate{};
    }

    void JsonReader::push_settings()
    {        
        Settings settings;
        ...
        settings.underlayer_color = color(dict.at("underlayer_colors"));
        settings.underlayer_width = dict.at("underlayer_width").AsDouble();
        for (const auto& color_ : dict.at("color_palette").AsArray()) {
